...
func (c *Client) res(req Request) (*string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("api2captcha: (%d) %s: %w", resp.StatusCode, data, ErrApi)
	}

	if strings.HasPrefix(data, "ERROR_") {
		return nil, fmt.Errorf("api2captcha: %s: %w", data, ErrApi)
	}
...
func (c *Client) Send(req Request) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("api2captcha: (%d) %s: %w", resp.StatusCode, data, ErrApi)
	}

	if strings.HasPrefix(data, "ERROR_") {
		return "", fmt.Errorf("api2captcha: %s: %w", data, ErrApi)
	}
